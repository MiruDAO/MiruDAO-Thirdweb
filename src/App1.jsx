...
const App = () => {
                    ...
                    try {
                      ...
                      await Promise.all(
                        votes.map(async (vote) => {
                          ...
                          if (proposal.state === 4) {
                            console.log("good proposal", proposal);
  ...
  const mintNft = () => {
    setIsClaiming(true);
    // claim("0", 1) mints one of the membership nft to the user's wallet
    bundleDropModule
      .claim("0", 1)
      .catch((err) => {
      ...
      .finally(() => {
        ...
        // Show user new NFT
        console.log(
          `😰 Succesfully Minted! Check it out on OpenSea: https://testnets.opensea.io/assets/${bundleDropModule.address}/0`
        );
      });
  };
